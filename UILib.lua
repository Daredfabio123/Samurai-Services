...
local SamuraiLibGui = game:GetObjects("rbxassetid://4530443679")[1];
SamuraiLibGui.Parent = game:GetService("CoreGui");
SamuraiLibGui.Name = httpService:GenerateGUID(false);
local mainFrame = SamuraiLibGui.mainFrame;
...
inputService.InputBegan:Connect(function(input, onGui)
    if not onGui and (input.KeyCode == Enum.KeyCode.P or input.KeyCode == Enum.KeyCode.RightShift) then
        SamuraiLibGui.Enabled = not SamuraiLibGui.Enabled;
    end;
end);

function SamuraiLib:SetCategory() end;

function SamuraiLib.Content:Resize(scrollingFrame)
    scrollingFrame.CanvasSize = newUDim2(0, 0, 0, (#scrollingFrame:GetChildren() - 1) * 36);
end;

function SamuraiLib.Content:Ripple(btn)
    spawn(function()
...
function SamuraiLib.Content:initBtnEffect(btn)
...
function SamuraiLib.Content:newBtn(title, callback, noToggle)
...
function SamuraiLib.Content:newSlider(title, callback, min, max, startPoint)
    self:Resize(self.bodyFrame);
...
function SamuraiLib.Content:newTextbox(title, callback, presetText, noCallbackOnStart)
    self:Resize(self.bodyFrame);
...
function SamuraiLib.Content:newBind(title, callback, presetKeyCode)
    self:Resize(self.bodyFrame);
...
function SamuraiLib.Content:newCBind(title, callback, presetKeyCode)
    self:Resize(self.bodyFrame);
...
function SamuraiLib.Content:newColorPicker(title, callback, presetColor)
    self:Resize(self.bodyFrame);
...
function SamuraiLib.Content:newDropdown(title, callback, list, noCallbackOnStart)
    self:Resize(self.bodyFrame);
